...
func getXmpModel(t *testing.T) *XmpEditor.Document {
	bytes := getAssetBytes(XmpFile, t)
	model := &XmpEditor.Document{}
	err := XmpEditor.Unmarshal(bytes, model)
	if err != nil {
...
func TestMetaDataEditor_DropXmp(t *testing.T) {
	...
	err := mde.DropXmp()
	if err != nil {
		t.Fatalf("error droppging XmpEditor data: %v", err)
	}
	//make sure segments are dropped
	if mde.HasXmp() {
		t.Errorf("Image still contains IPTC data")
	...
	b, err := mde.Bytes()
	if err != nil {
		t.Fatalf("could not write bytes after drop XmpEditor: %v", err)
	}
	mde, err = NewMetaDataEditor(b)
	if err != nil {
		t.Fatalf("could not open editor after dropping XmpEditor: %v ", err)
	}
	if mde.HasXmp() {
		t.Errorf("Image still contains IPTC data after writing bytes")
	}
...
func TestMetaDataEditor_HasXmp(t *testing.T) {
	...
	if !withMetaData.HasXmp() {
		t.Errorf("expected XmpEditor")
	}
	if noMetaData.HasXmp() {
		t.Errorf("expected no XmpEditor")
	}
}
...
func TestMetaDataEditor_SetIfdTag(t *testing.T) {
	...
	//TODO: make sure to cover all various types as well non-happy paths
	//TODO: this is broken. Fix so its pulling the actual lens info value and not the summary which
...
func TestMetaDataEditor_SetXmp(t *testing.T) {
	...
	if err := mde.SetXmp(model); err != nil {
		t.Fatalf("Could not set XmpEditor model")
	}
	md := editorMD(mde, t)
	if dcm := md.xmpData.DublinCore(); dcm != nil {
		if dcm.Title.Default() != expTitle {
